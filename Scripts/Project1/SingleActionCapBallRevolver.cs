...
{
    public class SingleActionCapBallRevolver : FVRFireArm
    {
        ...
        public SingleActionCapBallRevolverCylinder Cylinder;
        ...
        public int accessibleChamberOffset = 1;

        public bool allNipplesAccessible = false;
        ...
        public FVRFireArmChamber shotgunNipple;
        ...
        public int PrevChamber
        {
            get
            {
                int num = this.m_curChamber - accessibleChamberOffset;
        ...
        public override void Awake()
        {
            ...
            if (this.allNipplesAccessible)
            {
                for (int index = 0; index < this.Cylinder.Nipples.Length; ++index)
                    this.Cylinder.Nipples[index].IsAccessible = true;
            }
            if (this.allChambersAccessible)
            {
                for (int index = 0; index < this.Cylinder.Chambers.Length; ++index)
                    this.Cylinder.Chambers[index].IsAccessible = true;
            }
        }
        ...
        public override void UpdateInteraction(FVRViveHand hand)
        {
            ...
            if (!this.IsAltHeld)
            {
                if (!this.m_isStateToggled)
                {
                    if (hand.Input.TouchpadPressed && !hand.IsInStreamlinedMode && (double)Vector2.Angle(hand.Input.TouchpadAxes, Vector2.up) < 45.0 && !m_isHammerCocked)
                        this.m_isSpinning = true;
                    if (hand.IsInStreamlinedMode)
                    {
                        if (hand.Input.AXButtonDown && !this.m_isHammerCocked)
                            this.CockHammer(5f);
                        if (hand.Input.AXButtonDown && this.m_isHammerCocked)
                            this.SwitchFireMode();
        ...
        public override void FVRUpdate()
        {
            ...
            if (Cylinder.Chambers[RammingChamber] != null)
            {
                if (Cylinder.Chambers[RammingChamber].IsFull && lerp > 0f)
                {

                    Cylinder.RamChamber(RammingChamber, lerp);
                }
            }
            base.FVRUpdate();
        }
        ...
        private void Fire()
        {
            ...
            bool capFired = this.Cylinder.Nipples[this.CurChamber].Fire();
            ...
            if (!shotgunActive)
            {
                ...
                this.GasOutEffects[0].MaxGasRate = 12;
            ...
            } else
            {
                ...
                this.GasOutEffects[0].MaxGasRate = 0;
                capFired = this.shotgunNipple.Fire();
            ...
            if (GM.CurrentSceneSettings.IsAmmoInfinite && GM.CurrentPlayerBody.IsInfiniteAmmo)
            {
                ...
                this.Cylinder.Nipples[this.CurChamber].IsSpent = false;
            ...
            else
            {
                ...
                this.Cylinder.ChamberRammed(this.CurChamber, true, false);
            }
        }
        ...
        private void UpdateCylinderRot()
        {
            if (this.m_isStateToggled)
            {
                ...
                {
                    for (int index = 0; index < this.Cylinder.Nipples.Length; ++index)
                        this.Cylinder.Nipples[index].IsAccessible = index == num;
                }
            ...
            else
            {
                ...
                if (!this.allNipplesAccessible) { 
                    for (int index = 0; index < this.Cylinder.Nipples.Length; ++index)
                        this.Cylinder.Nipples[index].IsAccessible = false;
                }
