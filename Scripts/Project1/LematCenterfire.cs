...
{
    public class LematCenterfire : FVRFireArm
    {
        ...
        [Header("LeMat Config")]
        public float hammerUncockedRegular;
        public float hammerUncockedFlipped;
        public Transform hammerSwitch;
        public float switchFlipped;
        public float switchRegular;
        public Transform shotgunGate;
        public float shotgunGateOpen;
        public float shotgunGateClosed;
        ...
        public FVRFirearmAudioSet shotgunAudioSet;
        public FVRFirearmAudioSet regularAudioSet;
        public FVRFireArmRecoilProfile shotgunRecoil;

        public bool isEjecting = false;
        private bool shotgunActive = false;
        private bool isShotgunGateOpen = false;
        ...
        public override void UpdateInteraction(FVRViveHand hand)
        {
            ...
            if (!this.IsAltHeld)
            {
                if (!this.m_isStateToggled)
                {
                    if (hand.Input.TouchpadPressed && !hand.IsInStreamlinedMode && (double)Vector2.Angle(hand.Input.TouchpadAxes, Vector2.up) < 45.0 && !this.m_isHammerCocked)
                        this.m_isSpinning = true;
                    if (hand.IsInStreamlinedMode)
                    {
                        if (hand.Input.AXButtonDown)
                            this.CockHammer(5f);
                        if (hand.Input.BYButtonDown && this.StateToggles && !this.m_isHammerCocked)
                        {
                            this.ToggleState();
                            this.PlayAudioEvent(FirearmAudioEventType.BreachOpen);
                        }
                        if (hand.Input.AXButtonDown && this.m_hammerCockLerp == 1)
                        {
                            this.SwitchFireMode();
                            this.PlayAudioEvent(FirearmAudioEventType.Safety);
                        }
                        if (!this.isShotgunGateOpen)
                        {
                            if (hand.Input.BYButtonDown && this.StateToggles && this.m_isHammerCocked)
                            {
                                this.ToggleShotgunGate();
                                this.PlayAudioEvent(FirearmAudioEventType.BreachOpen);
                            }
                        } else
                        {
                    ...
                    else if (hand.Input.TouchpadDown)
                    {
                        if ((double)Vector2.Angle(hand.Input.TouchpadAxes, Vector2.down) < 45.0)
                            this.CockHammer(5f);
                        else if ((double)Vector2.Angle(hand.Input.TouchpadAxes, Vector2.left) < 45.0 && this.StateToggles && !this.m_isHammerCocked)
                        {
                            this.ToggleState();
                            this.PlayAudioEvent(FirearmAudioEventType.BreachOpen);
                        }
                        else if(hand.Input.TouchpadNorthPressed && this.m_isHammerCocked)
                        {
                            this.SwitchFireMode();
                            this.PlayAudioEvent(FirearmAudioEventType.Safety);
                        }
                        if (!this.isShotgunGateOpen)
                        {
                            if ((double)Vector2.Angle(hand.Input.TouchpadAxes, Vector2.left) < 45.0 && this.StateToggles && this.m_isHammerCocked)
                            {
                                this.ToggleShotgunGate();
                                this.PlayAudioEvent(FirearmAudioEventType.BreachOpen);
                            }
        ...
        public void ToggleShotgunGate()
        {
            this.isShotgunGateOpen = !isShotgunGateOpen;
            if (this.isShotgunGateOpen)
            {
                this.shotgunGate.localEulerAngles = new Vector3(0f, 0f, this.shotgunGateOpen);
                this.shotgunChamber.EjectRound(this.shotgunChamber.transform.position + this.shotgunChamber.transform.forward * -0.008f, -this.shotgunChamber.transform.forward * 0.002f, Vector3.zero);
                StartCoroutine(makeShotgunChamberAccessible());

            } else
            {
                this.shotgunGate.localEulerAngles = new Vector3(0f, 0f, this.shotgunGateClosed);
                this.shotgunChamber.IsAccessible = false;
            }

        }

        IEnumerator makeShotgunChamberAccessible()
        {
        ...
        {
            this.shotgunActive = !this.shotgunActive;
            if (shotgunActive)
            {
                this.Hammer_Rot_Uncocked = this.hammerUncockedFlipped;
                this.hammerSwitch.localEulerAngles = new Vector3(this.switchFlipped, 0f, 0f);
            } else
            {
                this.Hammer_Rot_Uncocked = this.hammerUncockedRegular;
                this.hammerSwitch.localEulerAngles = new Vector3(this.switchRegular, 0f, 0f);
            }
        }
            ...
            if (this.IsHeld)
            {
                if (this.m_isStateToggled)
                {
                    if (this.m_hand.Input.TriggerPressed)
                    {
                        ...
                        this.EjectorRod.transform.localPosition = Vector3.MoveTowards(this.EjectorRod.transform.localPosition, new Vector3(this.EjectorRod.transform.localPosition.x, this.EjectorRod.transform.localPosition.y, this.EjectorRod_Pos_Rearward.z), step);
                    }
                    else if (this.m_hand.Input.TriggerUp && this.EjectorRod.transform.localPosition != this.EjectorRod_Pos_Forward)
                    {
                        ...
                        this.EjectorRod.transform.localPosition = new Vector3(this.EjectorRod.transform.localPosition.x, this.EjectorRod.transform.localPosition.y, this.EjectorRod_Pos_Forward.z);
                    }
                }
            }
        }
        ...
        private void UpdateTriggerHammer()
        {
            ...
            this.Hammer.localEulerAngles = this.m_isStateToggled ? new Vector3(this.Hammer_Rot_Uncocked, 0.0f, 0.0f) : new Vector3(Mathf.Lerp(this.Hammer_Rot_Uncocked, this.Hammer_Rot_Cocked, this.m_hammerCockLerp), 0.0f, 0.0f);
        ...
        public void EjectPrevCylinder()
        {
            ...
            chamber.EjectRound(chamber.transform.position + chamber.transform.forward * -0.005f, -chamber.transform.forward, Vector3.zero);
        }
        ...
        private void Fire()
        {
            ...
            FVRFireArmRecoilProfile overrideRecoil = this.RecoilProfile;
            this.AudioClipSet = this.regularAudioSet;
            if (!this.shotgunActive)
            {
            ...
            {
                this.AudioClipSet = this.shotgunAudioSet;
                overrideRecoil = this.shotgunRecoil;
                ...
                if (!this.shotgunChamber.Fire() || this.isShotgunGateOpen)
                {
                    return;
                }
                chamber = this.shotgunChamber;
            }
            this.Fire(chamber, this.overrideMuzzle, true);
            this.FireMuzzleSmoke();
            this.Recoil(this.IsTwoHandStabilized(), (Object)this.AltGrip != (Object)null, this.IsShoulderStabilized(), overrideRecoil);
